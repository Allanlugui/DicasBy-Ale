...
export function Support() {
  ...
  useEffect(() => {
    if (tickets.length > 0 && !activeTicketId && !manuallyClosed) {
      ...
      const firstActive =
        tickets.find((t) => t.status !== "CLOSED") || tickets[0];
  ...
  useEffect(() => {
    if (!activeTicketId || !activeTicket || activeTicket.status === "CLOSED")
      return;

    const interval = setInterval(() => {
      const now = Date.now();
      const hasAgentJoined = activeTicket.messages.some(
        (m) => m.role === "bot" && m.isAgent === true,
      );
      ...
      if (activeTicket.needsHuman || hasAgentJoined) {
        ...
        const lastAgentMsgIndex = reversedMessages.findIndex(
          (m) => m.role === "bot" && m.isAgent === true,
        );
        ...
        if (lastAgentMsgIndex !== -1) {
          ...
          const clientRespondedAfter = reversedMessages
          ...
          if (!clientRespondedAfter && now - agentSentTime >= 600000) {
            ...
            updateTicket(
              activeTicket.id,
              [...activeTicket.messages, timeoutMsg],
              "CLOSED",
            );
  ...
  if (!user) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-16 text-center">
        <h2 className="text-2xl font-bold font-display text-stone-900 mb-4">
          Acesso Restrito
        </h2>
        <p className="text-stone-500 mb-6">
          Você precisa estar logado para acessar o Suporte Personalizado.
        </p>
        <a
          href="/login"
          className="bg-rose-600 text-white px-6 py-3 rounded-lg font-bold"
        >
          Fazer Login
        </a>
      </div>
  ...
  const handleSend = async (text: string) => {
    ...
    const isFirstUserMessage =
      activeTicket.messages.filter((m) => m.role === "user").length === 0;
    ...
    if (attachmentsToSave.length > 0) {
      for (const att of attachmentsToSave) {
        try {
          ...
          console.error(
            "Failed to auto-save file document into active user drive:",
            saveErr,
          );
        }
      }
    ...
    const hasAgentJoined = newMessages.some(
      (m) => m.role === "bot" && m.isAgent === true,
    );
      ...
      if (data.text) {
        ...
        if (cancelMatch) {
          ...
          textResult = textResult
            .replace(/\[CANCEL_ORDER_ID:\s*([^\]]+)\]/g, "")
            .trim();
          await updateOrderStatus(
            orderIdToCancel,
            "CANCELLED",
            "Cancelado via Atendimento Automático",
          );
        }
        ...
        if (transferMatch) {
          ...
          try {
            ...
            activeCollabs = collabSnap.docs
              .map((d) => d.data())
              .filter(
                (c: any) => c.permissions && c.permissions.includes("tickets"),
              );
          } catch (err) {
        ...
        <div className="flex flex-wrap gap-2">
          {msg.attachments.map((att, idx) => {
            ...
            return (
              <div
                key={idx}
                className="bg-white border border-stone-200 rounded-xl p-2 max-w-sm flex items-center gap-2.5 shadow-sm transition hover:shadow-md"
              >
                {isImg ? (
                  <a
                    href={att.url}
                    target="_blank"
                    rel="noreferrer"
                    className="shrink-0 group relative cursor-pointer block"
                  >
                    <img
                      src={att.url}
                      alt={att.name || "Image"}
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 grid grid-cols-1 md:grid-cols-3 gap-6 h-[80vh]">
      ...
      <div
        className={`bg-white rounded-2xl shadow-sm border border-stone-100 flex flex-col overflow-hidden h-full ${activeTicket && isExpanded ? "hidden" : "flex"}`}
      >
        <div className="p-4 border-b border-stone-100 bg-stone-50 flex justify-between items-center">
          <h2 className="font-bold text-stone-900 font-display">
            Meus Chamados
          </h2>
          <button
            onClick={() => startNewTicket()}
            className="bg-rose-100 text-rose-600 p-2 rounded-lg hover:bg-rose-200 cursor-pointer"
          >
            ...
            >
              <div className="flex justify-between items-center mb-1">
                ...
                <div className="flex items-center gap-1.5">
                  {t.status === "CLOSED" ? (
                    <span className="text-[9px] px-1.5 py-0.5 rounded font-bold uppercase bg-stone-100 text-stone-500 font-sans">
                      Encerrado
                    </span>
                  ) : (
                    <span className="text-[9px] px-1.5 py-0.5 rounded font-bold uppercase bg-semibold bg-emerald-50 text-emerald-600 border border-emerald-100 font-sans">
                      Ativo
                    </span>
                  )}
      ...
      {/* Chat Area */}
      <div
        className={`bg-white rounded-2xl shadow-sm border border-stone-100 flex flex-col overflow-hidden h-full ${activeTicket && isExpanded ? "md:col-span-3" : "md:col-span-2"}`}
      >
        {!activeTicket ? (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center text-stone-500">
            ...
            <p className="mb-4">
              Selecione um chamado ou inicie um novo para falar com nossa
              equipe/bot.
            </p>
            <button
              onClick={() => startNewTicket()}
              className="bg-rose-600 text-white font-bold py-2 px-6 rounded-full"
            >
          ...
          <>
            <div
              ...
              className={`p-4 border-b border-stone-100 bg-stone-50 flex justify-between items-center gap-2 ${!isExpanded ? "cursor-pointer hover:bg-stone-100 transition" : ""}`}
            >
              <div className="flex flex-col min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-bold text-stone-900 font-display truncate">
            ...
            <div
              ...
              className={`flex-1 overflow-y-auto p-4 space-y-4 ${!isExpanded ? "cursor-pointer" : ""}`}
            >
              {activeTicket.messages.map((m, i) => (
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${m.role === "user" ? "bg-stone-200 text-stone-600" : "bg-rose-500 text-white"}`}
                  >
                    {m.role === "user" ? (
                  ...
                  <div
                    className={`p-3 rounded-2xl max-w-[80%] ${m.role === "user" ? "bg-stone-100 text-stone-800 rounded-tr-sm" : "bg-rose-50 text-stone-900 rounded-tl-sm"}`}
                  >
              ...
              {activeTicket.status !== "CLOSED" &&
                activeTicket.messages.length === 1 && (
                  <div className="flex flex-wrap gap-2 mt-4 ml-11">
                    <button
                      onClick={() => handleSend("Cancelamento de compra")}
                      className="bg-white border border-stone-200 text-stone-700 text-xs py-2 px-4 rounded-full shadow-sm hover:bg-stone-50"
                    >
                      Cancelamento de compra
                    </button>
                    <button
                      onClick={() => handleSend("Dúvidas sobre prazos")}
                      className="bg-white border border-stone-200 text-stone-700 text-xs py-2 px-4 rounded-full shadow-sm hover:bg-stone-50"
                    >
                      Dúvidas sobre prazos
                    </button>
                    <button
                      onClick={() => handleSend("Outros")}
                      className="bg-white border border-stone-200 text-stone-700 text-xs py-2 px-4 rounded-full shadow-sm hover:bg-stone-50"
                    >
                      Outros
                    </button>
              ...
              <div className="p-4 bg-stone-50 border-t border-stone-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-stone-500">
                    Este bate-papo foi finalizado. Abra um novo chamado caso
                    precise de ajuda.
                  </span>
                </div>
                <button
                ...
                {selectedAttachments.length > 0 && (
                  <div className="flex flex-wrap gap-2 pb-2">
                    {selectedAttachments.map((att, idx) => (
                      <div
                        key={idx}
                        className="relative bg-stone-50 border border-stone-200 rounded-lg p-1.5 pr-8 flex items-center gap-1.5 text-xs max-w-xs shadow-sm"
                      >
                        {att.type.startsWith("image/") ? (
                          <img
                            src={att.url}
                ...
                >
                  ...
                  <div className="relative flex-grow flex items-center">
                    ...
                    <button
                      ...
                      className="absolute right-2 p-2 text-rose-500 hover:text-rose-600 disabled:opacity-50 cursor-pointer"
                    >
