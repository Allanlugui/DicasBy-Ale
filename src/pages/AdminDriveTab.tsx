...
export function AdminDriveTab({ limitToUserId }: { limitToUserId?: string }) {
  ...
  const folders = useMemo(
    () =>
      limitToUserId
        ? allFolders.filter(
            (f) =>
              f.userId === limitToUserId ||
              f.id === `root_${limitToUserId}` ||
              f.parentId?.startsWith(`root_${limitToUserId}`),
          )
        : allFolders,
    [allFolders, limitToUserId],
  );
  const documents = useMemo(
    () =>
      limitToUserId
        ? allDocuments.filter((d) => d.userId === limitToUserId)
        : allDocuments,
    [allDocuments, limitToUserId],
  );
  ...
  const handleMove = async (e: React.FormEvent) => {
    ...
    if (movingItem.type === "folder") {
      ...
      await updateDocument(movingItem.id, {
        folderId: finalTargetId || undefined,
      });
    }
    setMovingItem(null);
  };
    ...
    <div className="space-y-6">
      ...
      {showCreateFolder && (
        <form
          ...
          className="bg-white p-4 rounded-xl border border-stone-200 flex gap-3 items-end animate-fade-in"
        >
          ...
          <button
            type="submit"
            className="bg-stone-900 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-stone-800 transition"
          >
            Criar
          </button>
          <button
            ...
            className="bg-white text-stone-500 border border-stone-200 px-4 py-2 rounded-lg text-sm font-bold hover:bg-stone-50 transition"
          >
            Cancelar
          </button>
        </form>
      ...
      {showUploadDoc && (
        <form
          ...
          className="bg-white p-5 rounded-2xl border border-stone-200 space-y-4 animate-fade-in shadow-sm"
        >
          <h3 className="font-bold text-stone-900 border-b border-stone-100 pb-2">
            Novo Documento
          </h3>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
          ...
          {currentFolders.map((folder) => (
            ...
            >
              ...
              <span className="text-xs font-bold text-stone-700 text-center line-clamp-2 w-full px-1">
                {folder.name}
              </span>
          ...
          {currentDocuments.map((doc) => (
            ...
            >
              ...
              <div className="w-full text-center">
                <span
                  className="text-xs font-bold text-stone-700 block truncate"
                  title={doc.name}
                >
                  {doc.name}
                </span>
      ...
      {movingItem && (
        <div className="fixed inset-0 bg-stone-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <form
            onSubmit={handleMove}
            className="bg-white rounded-2xl max-w-md w-full shadow-2xl overflow-hidden animate-fade-in border border-stone-200"
          >
                ...
                >
                  ...
                  {folders.map((f) => (
                    <option
                      ...
                      value={f.id}
                      disabled={
                        movingItem.type === "folder" && f.id === movingItem.id
                      }
                    >
                      {Array((breadcrumbs.length > 0 ? 1 : 0) + 1)
                        .fill("—")
                        .join("")}{" "}
                      📁 {f.name}
                    </option>
            ...
            <div className="p-4 border-t border-stone-100 flex justify-end gap-2 bg-stone-50">
              ...
              <button
                type="submit"
                className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-lg text-sm font-bold shadow-sm transition"
              >
                Mover Item
              </button>
            </div>
