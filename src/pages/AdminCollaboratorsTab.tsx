...
export function AdminCollaboratorsTab() {
  ...
  const [role, setRole] = useState<
    ...
    | "OTHER"
  >("SUPPORT");
  const [receiveQuoteNotifications, setReceiveQuoteNotifications] =
    useState(false);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (snap) => {
        setCollaborators(
          snap.docs.map((d) => ({ id: d.id, ...d.data() }) as Collaborator),
        );
        setLoading(false);
      },
      (err) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data: Collaborator = {
        ...
        createdAt: editingId
          ? collaborators.find((c) => c.id === editingId)?.createdAt ||
      ...
      if (editingId && editingId !== id) {
        try {
          ...
          console.error(
            "Failed to clean up old collaborator document ID:",
            delErr,
          );
        }
      }
  ...
  const getRoleLabel = (r: string) => {
    switch (r) {
      ...
      case "SUPPORT":
        return "Atendimento/Suporte";
      case "LOGISTICS":
        return "Logística/Despacho";
      case "PACKAGING":
        return "Embalador/Separador";
      case "SALES":
  ...
  return (
    <div className="space-y-8 animate-fade-in" id="collaborators-tab-container">
      <div className="flex justify-between items-center flex-wrap gap-4">
        <div>
          ...
          <p className="text-xs text-stone-500">
            Cadastre e delegue funções específicas para sua equipe
          </p>
        </div>
      ...
      {showForm && (
        <form
          onSubmit={handleSave}
          className="bg-white border border-stone-200 rounded-2xl p-6 md:p-8 space-y-6 max-w-2xl shadow-sm transition"
        >
          ...
          <div className="space-y-3 pt-2">
            <label className="block text-xs font-bold text-stone-600 uppercase">
              Delegar Permissões (Funções no Sistema)
            </label>
            <p className="text-xs text-stone-400 mt-1">
              Marque cada módulo do sistema que este colaborador será autorizado
              a gerenciar
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-stone-50 p-4 rounded-2xl border border-stone-100">
              {availablePermissions.map((p) => (
                <label
                  key={p.key}
                  className="flex items-center gap-2 px-3 py-2 bg-white rounded-xl border border-stone-200 hover:bg-stone-50 cursor-pointer select-none"
                >
                  <input
                    type="checkbox"
          ...
          <div className="space-y-3 pt-2">
            ...
            <p className="text-xs text-stone-400 mt-1">
              Configure o recebimento de alertas automáticos para este
              colaborador
            </p>
            <div className="bg-stone-50 p-4 rounded-2xl border border-stone-100">
              <label className="flex items-start gap-3 px-3 py-3 bg-white rounded-xl border border-stone-200 hover:bg-stone-50 cursor-pointer select-none">
                <input
                  ...
                  onChange={(e) =>
                    setReceiveQuoteNotifications(e.target.checked)
                  }
                ...
                <div>
                  <span className="text-sm font-bold text-stone-700 block">
                    Solicitações de Orçamentos (Compras)
                  </span>
                  <span className="text-xs text-stone-400 block mt-0.5">
                    Receber e-mails e resumos em tempo real sempre que um
                    cliente fizer uma nova solicitação de orçamento de
                    importação.
                  </span>
                </div>
              </label>
      ...
      {loading ? (
        ...
        <div className="bg-stone-50 border border-stone-200 rounded-2xl p-12 text-center text-stone-500">
          ...
          <p className="text-xs text-stone-400 mt-1">
            Sua empresa ainda não possui colaboradores registrados neste
            sistema.
          </p>
        </div>
      ) : (
              ...
              <tbody className="divide-y divide-stone-100 text-sm">
                {collaborators.map((c) => (
                  ...
                  >
                    ...
                    <td className="p-4">
                      <span
                        className={`px-2.5 py-1 rounded-full text-xs font-bold ${getRoleBadge(c.role)}`}
                      >
                        {getRoleLabel(c.role)}
                      </span>
                    </td>
                        ...
                        {c.permissions && c.permissions.length > 0 ? (
                          c.permissions.map((p) => {
                            ...
                            return (
                              <span
                                key={p}
                                className="bg-stone-100 border border-stone-200 text-stone-600 text-[10px] font-bold px-1.5 py-0.5 rounded-lg"
                              >
                                {found?.label || p}
                              </span>
                            );
