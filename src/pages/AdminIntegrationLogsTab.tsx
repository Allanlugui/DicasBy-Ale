...
export function AdminIntegrationLogsTab() {
  ...
  const [serviceFilter, setServiceFilter] = useState<
    "ALL" | "Vendas" | "Finanças" | "Recursos Humanos"
  >("ALL");
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (snap) => {
        setLogs(
          snap.docs.map((d) => ({ id: d.id, ...d.data() }) as IntegrationLog),
        );
        setLoading(false);
      },
      (err) => {
  ...
  const handleClearAllLogs = async () => {
    if (
      window.confirm(
        "Tem certeza absoluta que deseja limpar todo o histórico e logs de auditoria de integrações? Esta ação é irreversível.",
      )
    ) {
      try {
        setLoading(true);
      ...
      <div className="flex justify-between items-center flex-wrap gap-4">
        <div>
          ...
          <p className="text-xs text-stone-500">
            Módulo de monitoramento em tempo real para as APIs do Nexus ERP e
            AdminHub
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white border border-stone-200 rounded-2xl p-5 shadow-xs">
          ...
          <span className="text-[10px] text-stone-400 mt-2 block">
            Sincronizações recebidas registradas no ERP
          </span>
        </div>
        <div className="bg-white border border-stone-100 rounded-2xl p-5 shadow-xs">
          ...
          <span className="text-[10px] text-emerald-600 font-medium mt-2 block flex items-center gap-1">
            <CheckCircle2 className="w-3.5 h-3.5" /> Transações operacionais
            executadas com sucesso
          </span>
        </div>
        <div className="bg-white border border-stone-200 rounded-2xl p-5 shadow-xs">
          <span className="text-xs text-stone-400 font-bold block uppercase tracking-wider">
            Erros & Rejeições de API
          </span>
          ...
          <span className="text-[10px] text-rose-500 font-medium mt-2 block flex items-center gap-1">
            <AlertCircle className="w-3.5 h-3.5" /> Rejeições por auth ou
            formatação (HTTP 400, 401)
          </span>
        </div>
      </div>
      ...
      <div className="bg-stone-50 border border-stone-200 rounded-2xl p-4 flex flex-col md:flex-row gap-4 items-center justify-between">
        <div className="flex flex-wrap gap-2 w-full md:w-auto">
          ...
          <button
            ...
            className={`px-3.5 py-1.5 rounded-lg text-xs font-bold transition-colors cursor-pointer ${
              serviceFilter === "Recursos Humanos"
                ? "bg-indigo-600 text-white shadow-xs"
                : "bg-white border border-stone-200 text-stone-600 hover:bg-stone-100"
            }`}
          >
        ...
        <div className="flex gap-4 items-center w-full md:w-auto justify-end flex-wrap">
          ...
          <div className="flex items-center gap-2 text-xs">
            <span className="font-bold text-stone-500 uppercase tracking-wider whitespace-nowrap">
              Status:
            </span>
            <select
              value={statusFilter}
      ...
      {loading ? (
        <div className="py-16 text-center text-stone-400 flex flex-col items-center gap-3">
          ...
          <span className="text-xs font-semibold">
            Buscando banco de auditorias ativas...
          </span>
        </div>
        ...
        <div className="bg-stone-50 border border-dashed border-stone-300 rounded-2xl p-12 text-center text-stone-500">
          ...
          <p className="text-xs text-stone-400 mt-1">
            Não existem requisições que correspondam aos filtros de busca
            atuais.
          </p>
        </div>
      ) : (
              ...
              <tbody className="divide-y divide-stone-100 text-xs">
                {filteredLogs.map((log) => (
                  ...
                  >
                    ...
                    <td className="p-4">
                      <span
                        className={`px-2.5 py-0.5 border rounded-full text-[10px] font-bold ${getServiceBadge(log.service)}`}
                      >
                        {log.service}
                      </span>
                    </td>
                    ...
                    <td className="p-4 text-center">
                      <span
                        className={`inline-flex items-center gap-1 border px-2 py-0.5 rounded-lg font-mono text-[11px] font-black ${getStatusBadge(log)}`}
                      >
                        {log.status === "SUCCESS" ? (
                    ...
                    <td className="p-4 max-w-xs font-sans text-stone-600 truncate">
                      {log.status === "SUCCESS" ? (
                        ...
                        <span
                          className="text-rose-600 font-bold block truncate"
                          title={
                            log.errorDescription || "Erro não especificado"
                          }
                        >
                          ⚠️{" "}
                          {log.errorDescription ||
                            "Rejeição de formato de dados / payload incorreto"}
                        </span>
                      )}
                    </td>
        ...
        <div className="fixed inset-0 bg-stone-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white rounded-2xl border border-stone-200 p-6 max-w-2xl w-full max-h-[85vh] overflow-y-auto space-y-6 shadow-xl relative animate-scale-up">
            ...
            <div className="flex justify-between items-start border-b border-stone-100 pb-4">
              <div>
                <span className="text-[10px] font-mono font-black text-stone-400 block tracking-widest uppercase">
                  Protocolo de Auditoria Ativa
                </span>
                ...
                <div className="flex gap-2 items-center mt-2 flex-wrap">
                  <span
                    className={`px-2 py-0.5 border rounded-full text-[10px] font-bold ${getServiceBadge(selectedLog.service)}`}
                  >
                    {selectedLog.service}
                  </span>
                  <span
                    className={`inline-flex items-center gap-1 border px-2 py-0.5 rounded-lg font-mono text-[10px] font-black ${getStatusBadge(selectedLog)}`}
                  >
            ...
            <div className="space-y-3">
              ...
              <div className="bg-stone-50 border border-stone-200 p-4 rounded-xl space-y-2 text-xs">
                <div>
                  ...
                  <span className="font-mono ml-2 font-semibold text-stone-700">
                ...
                <div>
                  ...
                  <span
                    className={`ml-2 font-bold ${selectedLog.status === "SUCCESS" ? "text-emerald-600" : "text-rose-600"}`}
                  >
            ...
            <p className="text-[10px] text-stone-400 text-center">
              Todos os payloads de integração são monitorados de forma segura
              sob as diretrizes de LGPD e autenticação HMAC-SHA256 de chaves de
              API.
            </p>
          </div>
        </div>
